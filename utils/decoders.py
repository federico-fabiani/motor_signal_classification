...
from tensorflow.keras.layers import Layer, InputLayer, Reshape, Permute, Dense, Dropout, SimpleRNN, GRU, LSTM, Conv2D, \
    MaxPool2D, Flatten, TimeDistributed, AvgPool2D, Conv1D, MaxPool1D, AvgPool1D, MultiHeadAttention, LayerNormalization
...
class Ensemble:
    ...
    dropout: decimal, optional, default 0
        Proportion of units that get dropped out
    """
    def __init__(self, channels, outputs, filters=10, filters_size=None, activation='relu', dropout=0, pooling_layer='max', recurrent_layer='lstm', 
                 recurrent_dropout=0, units=16, neurons=0, lr=0.001):
        self.name = 'ensemble'
        ...
        self.model.add(TimeDistributed(
            Conv1D(filters[0], filters_size[0], strides=filters_size[0], activation=activation)))
        for f, s in zip(filters[1:], filters_size[1:]):
            if f == 0:
                break
            self.model.add(TimeDistributed(Conv1D(f, s, activation=activation)))
        if dropout != 0:
            self.model.add(TimeDistributed(Dropout(dropout)))
        if pooling_layer == 'max':
            self.model.add(TimeDistributed(MaxPool1D(2)))
        ...
        if recurrent_dropout != 0:
                self.model.add(Dropout(recurrent_dropout))
        self.model.add(Flatten())
        # Add output layer
        if neurons != 0:
        ...
        # Fit model (and set fitting parameters)
        self.model.compile(loss='categorical_crossentropy', optimizer=Adam(learning_rate=lr), metrics=['accuracy'])
        self.initial_weights = self.model.get_weights()
    ...
    def fit(self, x_train, y_train, x_val=None, y_val=None, num_epochs=10, batch_size=0, verbose=0):
        ...
        if x_val is not None:
            ...
            history = self.model.fit(x_train, y_train, batch_size=batch_size, epochs=num_epochs, verbose=verbose, shuffle=True)
